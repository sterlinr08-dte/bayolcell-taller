...
- **Endpoints CONFIRMADOS que existen:**
  ...
  - `cliente` (GET) y `proveedor` (GET) — **el endpoint existe** pero la clave **aún NO tiene permiso** (devuelve "Error en las credenciales", igual que clave inválida, aunque artículos/ajuste sí funcionan). ⏳ Pedir a Dagoberto que **habilite `cliente` y `proveedor` para la clave**. Cuando los habilite, se sincronizan **igual que los artículos** (mismo método de `infoplus-sync`).
  - `ajuste` (POST) — **ajuste de inventario, PROBADO y funciona** ("Se ha creado correctamente"). ✅
  - ❌ NO existen con esos nombres (404): `clientes`, `listaclientes`, `proveedores`, `listaproveedores`, `suplidor`, `listasuplidores`, `compra`, `listacompras`. El nombre real de **compra** hay que pedírselo a Dagoberto.

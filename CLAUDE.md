...
- **Liquidación de Aduana** (en Compras, `v-proveedores`): calculadora **independiente** (NO guarda nada, NO se conecta a Supabase ni Info Plus). Botón morado "Liquidación Aduana" → modal `modalLiquidacionAduana`. Escribes/pegas los productos de la factura (descripción, cantidad, costo) y agregas **Transporte + Impuesto (monto fijo o % sobre factura) + Otros gastos**; reparte esos gastos **proporcional al importe** de cada producto y da el **costo final por producto/unidad**. Imprime y exporta a Excel (mismo estilo que los reportes de costos). Funciones: `abrirLiquidacionAduana()`, `_aduanaCalcular()` (núcleo del cálculo), `_aduanaRecalcular()` (refresca tabla+resumen sin perder foco), `_aduanaPegar()` (pegar desde Excel), `imprimirLiquidacionAduana()`, `exportarLiquidacionAduanaExcel()`. Estado en `_aduanaItems`. Pendiente: el usuario enviará el formato de factura del proveedor para calcar columnas exactas.

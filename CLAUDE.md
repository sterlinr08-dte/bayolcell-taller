...
- **Convenciones NEXUS aplicadas al código nuevo:** sin emojis en la interfaz (solo íconos `ti-*` + texto), RD$/es-DO, imprimible+WhatsApp, RLS en la tabla nueva. **Deuda conocida:** el resto de `taller.html` (app vanilla vieja) todavía usa emojis y hex sueltos en toda la UI; alinearlo 100% a las convenciones NEXUS sería una migración grande aparte (el ecosistema NEXUS son las apps Vite+React; `taller.html` es la app legacy). Decidir con Sterling si/ cuándo se limpia.

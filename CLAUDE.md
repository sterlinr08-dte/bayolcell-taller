...
- **Endpoints CONFIRMADOS que existen:**
  ...
  - `cliente` (GET), `proveedor` (GET) e **`inventario` (GET)** — **el endpoint existe** pero la clave **aún NO tiene permiso** (devuelve "Error en las credenciales", aunque artículos/ajuste/factura sí funcionan). ⏳ Pedir a Dagoberto que **habilite `cliente`, `proveedor` e `inventario` para la clave**. `inventario` es el candidato más probable para **leer los seriales/IMEI** por artículo/almacén.
